...
const TRANSLATIONS = {
  ...
  'kpop.shareText': {
    en: 'I got {correct} out of {total} sentences from a BTS video! ({percent}% success) Try it yourself: {url}',
    ko: '나는 방탄소년단의 영상에서 총 {total}개 중 {correct}개를 맞혔어요! (성공률 {percent}%) 함께 도전해보세요: {url}',
    es: '¡Acerté {correct} de {total} frases de un video de BTS! ({percent}% de acierto) Inténtalo tú: {url}',
    ja: 'BTSの動画で全{total}問中{correct}問正解しました！（成功率{percent}%）あなたも挑戦してみて: {url}',
  },
