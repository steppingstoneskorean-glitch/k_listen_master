...
|---|---|---|
| 신고[신꼬] / 신문[신문] | `제24항` / `null` | 경음화 적용 여부 |
| 같이[가치] / 잔디[잔디] | `제17항` / `null` | 구개음화 적용 여부 |
| 꽃이[꼬치] / 꽃 위[꼬뒤] | `제13·14항` / `제15항` | 연음 vs 대표음 후 연음 |
| 신라[실라] / 정리[정니] | `제20항` / `제19항` | Flow 방향(ㄹ이김 vs ㄹ물러섬) |
...
const 신고_신문: ContrastSet = {
  ...
  members: [
    { ref: 101, firedRule: '제24항', surface: '신꼬' },              // 신고 (용언)
